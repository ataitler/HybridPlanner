...
class Grounder:
    ...
    def Ground(self, domain="", problem=""):
        ...
        for typestr in re.findall("(?s)\s*(.*?\-\s*\S*)", typesTxt):
            # splited = re.search("(?s)\s*(.*?)\s*\-\s*([a-zA-Z_]*)", typestr)
            splited = re.search("(?s)\s*(.*?)\s*\-\s*(\S*)", typestr)
            ...
            for child in re.findall("\S+", childs):
                p.AddChild(Node(child))
        print("DONE")

        m = re.search("(?s)(\(define.*\))\s*(\(:\s*\s*domain.*?\))\s*\(:\s*objects\s*(.*?)\s*\)(.*)", self.problemTxt)
        ...
        m_const = re.search("(?s)\s*\(:constants\s*(.*?)\s*\)", restDomain)
        constantsTxt = ""
        ...
        objectsTxt = objectsTxt + constantsTxt
        # for objlist in re.findall("(?s)\s*(.*?)\s+\-\s+([a-zA-Z_]*)", objectsTxt):
        for objlist in re.findall("(?s)\s*(.*?)\s+\-\s+(\S*)", objectsTxt):
            type = objlist[1]
        ...
        for action in re.findall("(?s)(?=(\(:durative-action.*?\(:))", unGacts):
            ...
            if andcond is None:
                condStrip = re.search('(.*?)\(\s*(.*?)\s*\)(.*?)\s*', conditions)
                conditions = condStrip.group(1) + "(" + condStrip.group(2) + ")" + condStrip.group(3)
