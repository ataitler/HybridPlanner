...
class Activity(PDDLElement):
    def __init__(self, name, regionName = None):
        PDDLElement.__init__(self, name)
        self.conditions = {}
        self.effects = {}
        self.type = None
...
class DurativeAction(Activity):
    def __init__(self, name, params, durs, conds, effs, regionName):
        Activity.__init__(self, name, regionName)
        self.params = params
        self.durations = durs
        self.conditions = conds
    ...
    def SetSystems(self, master, slaves):
        self.master = master
        if master:
            if len(master) > 0:
                if slaves:
    ...
    def ToPDDL(self):
        ...
        st = st.rstrip()
        st = st + ")\n"
        st = st + "\t:duration (and (>= ?duration " + self.durations[0] + ") (<= ?duration " + self.durations[1] + "))\n"
        st = st + "\t:condition (and\n"
